...
public class KinectManager : MonoBehaviour 
{
	...
	[Tooltip("Whether to utilize the user and depth maps.")]
	...
	[Tooltip("Whether to detect only closest user first or not.")]
	...
	private bool allowTurnArounds = false;
	...
	[Tooltip("List of the utilized gesture listeners. They must implement KinectGestures.GestureListenerInterface. If the list is empty, the available gesture listeners will be detected at start up.")]
...
//	}
	...
	/// <summary>
	/// Determines whether the user with the specified userId is calibrated or not, when a calibration pose is required.
	/// </summary>
	/// <returns><c>true</c> if the user with the specified userId is calibrated; otherwise, <c>false</c>.</returns>
	...
	void UpdateUserHistogramImage()
	{
		...
		for (int i = 0; i < usersMapSize; i++)
		{
			...
			if(nowUserPixel != wasUserPixel)
			{
				...
				bool bUserTracked = btSelBI != 255 ? btSelBI == (byte)userMap : alTrackedIndexes.Contains(userMap);
				if (userMap == 255 || !bUserTracked)
				{
...
////						"{0} - BodyAngle: {1:000}, FullAngle: {2:000}, NeckAngle: {3:000}", 
////						(!bodyData.isTurnedAround ? "FACE" : "BACK"), bodyTurnAngle, bodyData.bodyFullAngle, neckTiltAngle);
////				}
