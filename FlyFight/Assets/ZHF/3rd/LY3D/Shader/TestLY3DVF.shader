	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float4 clr1 = tex2D(_Tex1, i.uv);
				float4 clr2 = tex2D(_Tex2, i.uv);
				float4 clr3 = tex2D(_Tex3, i.uv);
				float4 clr4 = tex2D(_Tex4, i.uv);
				float4 clr5 = tex2D(_Tex5, i.uv);
				float4 clr6 = tex2D(_Tex6, i.uv);
				float4 clr7 = tex2D(_Tex7, i.uv);
				...
				float fRRatio = 0;
				float N;
				int xt = (int)(i.uv.x *_Width) * 3 + 2;
				int y = (int)((1 - i.uv.y) * _Height);
				...
				fixed4 col;
				col.r = (fR2*(1 - fRRatio) + fR1*fRRatio);
				col.g = (fG2*(1 - fGRatio) + fG1*fGRatio);
				col.b = (fB2*(1 - fBRatio) + fB1*fBRatio);
				col.a = 1;
				return col;
			}
			ENDCG
